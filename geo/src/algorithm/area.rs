...
use crate::geometry::*;
use crate::traits::{LineStringTrait, PolygonTrait};
...
pub(crate) fn twice_signed_ring_area_trait(linestring: impl LineStringTrait) -> f64 {
    // LineString with less than 3 points is empty, or a
    // single point, or is not closed.
    ...
    // Above test ensures the vector has at least 2 elements.
    // We check if linestring is closed, and return 0 otherwise.
    let p1 = linestring.point(0).unwrap();
    let p2 = linestring.point(linestring.num_points() - 1).unwrap();
...
pub(crate) fn get_linestring_area_trait(linestring: impl LineStringTrait) -> f64 {
    twice_signed_ring_area_trait(linestring) / (1_f64 + 1_f64)
}
...
pub fn area_polygon<'a>(geom: &'a impl PolygonTrait<'a>) -> f64 {
